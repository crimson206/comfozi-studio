...
__export(index_exports, {
  DEFAULT_PARSER_PROFILE: () => DEFAULT_PARSER_PROFILE,
  DETERMINISTIC_CHAIN: () => DETERMINISTIC_CHAIN,
  RunEventEmitter: () => RunEventEmitter,
  STAGE_LABEL: () => import_contract.STAGE_LABEL,
  STAGE_ORDER: () => import_contract.STAGE_ORDER,
  SessionPool: () => SessionPool,
  aggregate: () => aggregate,
  classifyFormat: () => classifyFormat,
  emptyRunState: () => import_contract.emptyRunState,
  extractOcrText: () => extractOcrText,
  foldRun: () => import_contract.foldRun,
  hasTesseract: () => hasTesseract,
  isDeterministicCandidate: () => isDeterministicCandidate,
  iseshTransport: () => iseshTransport,
  mapLimit: () => mapLimit,
  mergeRows: () => mergeRows,
  normalizeAiRows: () => normalizeAiRows,
  parseFleet: () => parseFleet,
  rasterizePdf: () => rasterizePdf,
  routeOne: () => routeOne,
  runDeterministicBatch: () => runDeterministicBatch,
  runDeterministicOne: () => runDeterministicOne,
  ...
  async submitBatch(docs, _opts) {
    ...
    try {
      ...
      return meta.map((m, i) => {
        ...
        const stamped = (0, import_doc_import2.stampParser)(rows, "vision-pool", { file: m.filename, confidence: minConfidence ?? 0.8 });
