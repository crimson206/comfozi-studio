...
# 3) parse-fleet 파서 프로필(배치 계약) 설치 — 배포된 skit (⚠ 버전 @0.1.0 명시)
skit install @comfozi/parse-fleet@0.1.0
#   (오프라인/대체: make install-parser 로 vendored 프로필 직접 복사)
```
...
- **`make parse MODE=ai` 가 `failed=전체`(ai=0):** `command -v tmux isesh claude`(빠지면 각각 설치) · `snapshot install @ist/beta` · `skit install @comfozi/parse-fleet@0.1.0`(또는 `make install-parser`) · `claude login` 확인. (`detector-agent ✗` 는 무관.)
